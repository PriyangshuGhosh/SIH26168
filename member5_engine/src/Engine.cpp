...
void Engine::handleImu(const ImuSample& s) {
    ...
    const float ch[kChannels] = {
        static_cast<float>(aligned.ax_v / 9.80665), static_cast<float>(aligned.ay_v / 9.80665),
        static_cast<float>(aligned.az_v / 9.80665), static_cast<float>(aligned.gx_v),
    ...
    if (speed_ && window_count_ >= model_T_ && stride_count_ >= kStride) {
        ...
        if (est.valid) {
            ...
            // ZUPT: If Member 2 proves we are static, clamp AI hallucination to 0.0
            if (aligned.status == sih26168::member2::CalibrationStatus::STATIC_DETECTED) {
                meas.velocity_mps = 0.0;
                meas.variance_m2s2 = 0.001; // extremely high confidence we are stopped
            } else {
