...
void gnss(){EKFFusionEngine e;e.updateGnss(gnss(0,17.385,78.4867,4));assert(e.state().valid);e.predict(imu(.1),NavigationMode::GNSS_AIDED);e.updateGnss(gnss(.1,17.3851,78.4868,4));assert(e.state().last_gnss_accepted);}
...
void ai(){EKFFusionEngine e;e.predict(imu(0),NavigationMode::DEAD_RECKONING);e.updateAiSpeed({1,5,.25,true});assert(e.state().last_ai_speed_accepted);double v=e.state().v_x;e.updateAiSpeed({1,80,.01,true});assert(!e.state().last_ai_speed_accepted);assert(std::abs(e.state().v_x-v)<1e-9);}
